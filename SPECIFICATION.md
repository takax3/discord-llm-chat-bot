...
- Discord の presence にモデル名・待機キュー数・直近のトークンスピードを表示する。モデル名が未設定の場合はモデル名部分を省略する。
...
10. Stage 2 としてメインモデルが最終回答を生成し、Discord に返信する。
    - ContextBuilder がシステムプロンプトを組み立てる。構成: `SYSTEM_PROMPT` + 現在日時（JST）+ ルール群。
    - ルール群には Discord Markdown 指示を含む。Discord でレンダリングされる装飾（`**bold**` / `*italic*` / `__underline__` / `~~strikethrough~~` / インラインコード / コードブロック / `>` 引用 / 箇条書き / 番号リスト / `#` `##` `###` 見出し）のみ使うよう指示し、HTML タグ・`---` 水平線・Markdown テーブル・タスクリスト・セテックス見出し・参照スタイルリンク・画像埋め込みは使わないよう指示する。`##` / `###` 見出しは長い構造化レスポンスのときのみ使うよう指示する。
...
- 動作:
  - 正常系:
    ...
    - `reply_sent_at` が記録されている場合は返信完了時刻を表示する。
    - `gpu_avg_watts` が記録されている場合は GPU 消費電力（W および kWh）を表示する。kWh は `gpu_energy_joules ÷ 3,600,000` で算出する。`gpu_avg_watts` が NULL の場合（NVIDIA Container Toolkit 未インストール等）は行ごと非表示にする。
  - ログ未存在:
